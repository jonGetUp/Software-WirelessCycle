...
//------------------------------------------------------------------------------
#define SIZE_OF_RX_BUFFER 8   ///< size of receive fifo buffer
#define SIZE_OF_TX_BUFFER 8   ///< size of transmit fifo buffer
...
struct CANMESSAGE
{
  ...
  uint8_t   filhit;		///< Accept. Filter that enabled the reception, 
                        ///  to find the message path without identifier
  uint8_t   txPrio;     ///< Send priority (0 to 3, 3 is max), NOT USED
};
...
{
  ...
  retCode = Can_GetMessage(&msg);  // read message
