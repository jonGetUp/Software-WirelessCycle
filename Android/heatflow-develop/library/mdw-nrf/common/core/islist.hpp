 ...
 * \file    islist.hpp
 * \class   IsList
 * \breif   IsList is a Interrupt Save List. It can  be called inside an ISR (Interrupt Service Routine).
 *
...
public:
    ...
    /**
     * @brief Initializes list with a memory pool.
     * The list can be initialized with an object pool. This causes the list to no
     * more dynamically allocate memory for the internal list structure.
     * @param pItemPool Pointer to object pool.
     */
    void initialize(ItemObjectPool* pItemPool)
    {
        _pItemPool = pItemPool;
    }

    inline bool isMemporyPoolPresent() const { return (_pItemPool != NULL); }
...
protected:
    ...
    Item* createItem(const T &c)
    {
        ...
        if(isMemporyPoolPresent())
        {
            // Create item using object pool
            if(_pItemPool->availableObjectCount() > 0)
            {
                pItem = _pItemPool->allocate();     // Get memory from pool
                if(pItem)
                {
                    pItem = new(*pItem) Item(c);    // Initialize item
                }
            }
        }
        else
